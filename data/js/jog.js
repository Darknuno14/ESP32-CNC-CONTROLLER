...
function updateMachineStatus(data) {
  ...
  switch(data.state) {
    case 0: statusText.textContent = 'Bezczynny'; break;
    case 1: statusText.textContent = data.isPaused ? 'Wstrzymany' : 'Pracuje'; break;
    case 2: statusText.textContent = 'Sterowanie JOG'; break;
    case 3: statusText.textContent = 'Bazowanie'; break;
    case 4: statusText.textContent = 'Zatrzymany'; break;
    case 5: statusText.textContent = 'Błąd'; break;
    default: statusText.textContent = 'Nieznany';
  }
...
function jog(xDir, yDir) {
  ...
  .then(data => {
    if (data.success) {
      ...
      showMessage('Nie udało się wykonać ruchu: ' + (data.message || 'Nieznany błąd'), 'error');
    }
  })
  .catch(error => {
...
function zeroAxes() {
  ...
  .then(data => {
    if (data.success) {
      ...
      showMessage('Nie udało się wyzerować pozycji: ' + (data.message || 'Nieznany błąd'), 'error');
    }
  })
  .catch(error => {
 ...
 */
function homeAxes() {
  if (!confirm('Czy na pewno chcesz wykonać bazowanie? Maszyna przesunie się do pozycji bazowej.')) {
    return;
  }
  ...
  .then(data => {
    if (data.success) {
      ...
      showMessage('Nie udało się uruchomić bazowania: ' + (data.message || 'Nieznany błąd'), 'error');
    }
  })
  .catch(error => {
...
function toggleWire() {
  ...
  .then(data => {
    if (data.success) {
      ...
      wireSwitch.checked = !wireState; // Przywróć poprzedni stan przełącznika
      showMessage('Nie udało się przełączyć drutu: ' + (data.message || 'Nieznany błąd'), 'error');
    }
  })
  .catch(error => {
...
function toggleFan() {
  ...
  .then(data => {
    if (data.success) {
      ...
      fanSwitch.checked = !fanState; // Przywróć poprzedni stan przełącznika
      showMessage('Nie udało się przełączyć wentylatora: ' + (data.message || 'Nieznany błąd'), 'error');
    }
  })
  .catch(error => {
...
function handleKeyboardControl(event) {
  ...
  switch (event.key) {
    ...
    case 'W':
      event.preventDefault();
      document.getElementById('wireSwitch').checked = !document.getElementById('wireSwitch').checked;
      toggleWire();
      break;
    case 'f':
    case 'F':
      event.preventDefault();
      document.getElementById('fanSwitch').checked = !document.getElementById('fanSwitch').checked;
      toggleFan();
      break;
  }
}
...
function selectNextDistance(direction) {
  ...
  // Wyświetl komunikat
  showMessage(`Zmieniono odległość ruchu na ${distanceRadios[nextIndex].value} mm`);
}
